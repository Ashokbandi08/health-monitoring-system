...
// Function to map raw values to medical-grade heart rate (BPM)
int mapHeartRate(long rawValue) {
    return map(rawValue, 50000, 120000, 60, 100); // Adjust based on sensor calibration
}

// Function to map raw values to medical-grade SpO2 (%)
int mapSpO2(long rawValue) {
...
void setup() {
    ...
    particleSensor.setPulseAmplitudeRed(0x0A); // Turn on LED at low level
...
void loop() {
    ...
    long rawHeartRate = particleSensor.getIR();
    long rawSpO2 = particleSensor.getRed();

    // Map values to medical-grade standards
    int heartRate = mapHeartRate(rawHeartRate);
    int spo2 = mapSpO2(rawSpO2);
